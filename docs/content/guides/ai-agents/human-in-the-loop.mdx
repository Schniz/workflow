...
An extremely common feature necessary for running AI agents in production is the ability to wait for human input or external events before proceeding.
...
<Step>
`createWebhook()` or `createHook()` generates a unique URL (or token) that can resume a workflow. This can be packaged as a tool for the Agent to call, or used directly in your backend code.
</Step>

<Step>
The workflow emits a data chunk containing the webhook URL, sending it to the consumer of the workflow, such as a chat UI, or an external service.
</Step>
...
<Step>
The consumer of the workflow calls the webhook URL, sending data back to the workflow.
</Step>

<Step>
The workflow receives any data sent with the webhook (approval status, comments) and resumes execution.
</Step>
