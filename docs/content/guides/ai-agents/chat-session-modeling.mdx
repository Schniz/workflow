...
This is the pattern used in the [Building Durable AI Agents](/guides/ai-agents) guide.
...
```typescript title="app/api/internal/order-update/route.ts" lineNumbers
import { chatMessageHook } from '@/ai/hooks/chat-message';
...
```typescript title="app/api/webhooks/stripe/route.ts" lineNumbers
import { chatMessageHook } from '@/ai/hooks/chat-message';
...
export async function POST(req: Request) {
  ...
  if (event.type === 'payment_intent.succeeded') {
    ...
    const threadId = paymentIntent.metadata.threadId; // [!code highlight]
