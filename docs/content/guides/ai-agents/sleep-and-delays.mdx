...
First, we make the `sleep()` function available to the AI Agent by packaging it as a tool.
...
We want the UI to display the waiting state, so we emit a new chunk type `data-wait`, that the UI can then consume.
